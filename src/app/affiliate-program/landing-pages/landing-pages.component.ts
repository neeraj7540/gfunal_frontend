...
export class LandingPagesComponent implements OnInit {
  ...
  onManage_Clicked(data) {
    this.router.navigate(['affiliate-dashboard'], { queryParams: { type: "landingPage", affilateId: data._id, link: data.link } });
  }

  onJoin_Clicked(data) {
    this.router.navigate(['terms'], { queryParams: { type: "landingPage", affilateId: data._id, link: data.link } });
  }
}
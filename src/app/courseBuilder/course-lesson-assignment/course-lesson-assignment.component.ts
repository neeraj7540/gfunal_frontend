...
@Component({
  selector: 'app-course-lesson-assignment',
  templateUrl: './course-lesson-assignment.component.html',
  styleUrls: ['./course-lesson-assignment.component.css']
})
export class CourseLessonAssignmentComponent implements OnInit {
    ...
    if(this.validation()){
      try{
        let param = {
          ...
          title:this.addPostParam.assignmentTitle,
          content:this.addPostParam.AssignmentContent,
          savedAs: true
        };
        this.api.createLessonAssignment(param,this.auth.getToken()).subscribe(response=>{
          if(!response['status'])
